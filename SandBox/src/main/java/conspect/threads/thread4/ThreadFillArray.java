...
public class ThreadFillArray implements Runnable {
    ...
    public ThreadFillArray(String direction, int[][] array, int iteration, CountDownLatch countDownLatch) {
    ...
    private void writeArray(int i, int j, int iteration) {
        if (array[i][j] == iteration) {
            try {
                if (array[i + 1][j] == -1) array[i + 1][j] = iteration + 1;
            ...
            try {
                if (array[i][j + 1] == -1) array[i][j + 1] = iteration + 1;
            } catch (ArrayIndexOutOfBoundsException e) {
